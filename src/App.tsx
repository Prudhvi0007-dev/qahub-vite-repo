...
import CreateTestPlanPage from "./CreateTestPlan";

// ------------------------------------------------------------
// QAHub — Check-style menu overlay (match reference spacing)
// Requirements implemented:
// - NO external font links (local @font-face only)
// - React Router routing + separate page components
// - Two-column menu layout with reference-like spacing
// - NO divider/underline lines under menu items
// - Unique hover color per item
// - Right panel behavior: tints based on hovered item color (fallback to active route)
// ------------------------------------------------------------
...
const MENU_ITEMS = [
  ...
  },
  {
    ...
    color: "#f59e0b",
    desc: "Create comprehensive test plans with checklists.",
  },
] as const;
  ...
  return (
    <style>{`
      :root{
        ...
        --font-condensed: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial;
      }
...
function MenuOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleItemHover = (key: MenuKey, element: HTMLButtonElement) => {
    setHoverKey(key);
    const gridContainer = element.closest('.grid');
    if (gridContainer) {
      const buttonRect = element.getBoundingClientRect();
      const gridRect = gridContainer.getBoundingClientRect();
      setPanelTop(buttonRect.top - gridRect.top);
    }
  };
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          <div className="relative mx-auto grid max-w-[min(1280px,calc(100vw-3rem))] grid-cols-12 gap-x-10 md:gap-x-14 lg:gap-x-20 gap-y-10 px-6 sm:px-8 md:px-12 lg:px-24 pb-8 pt-8 sm:pt-9 md:pt-10 overflow-y-auto max-h-[calc(100vh-120px)]">
            <div className="col-span-12 lg:col-span-8 min-w-0">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 sm:gap-y-12 md:gap-y-14 gap-x-12 md:gap-x-20 lg:gap-x-32 pb-8">
                {MENU_ITEMS.map((item, idx) => {
                  ...
                  return (
                    <motion.button
                      key={item.key}
                      onMouseEnter={(e) => handleItemHover(item.key, e.currentTarget)}
                      onFocus={(e) => handleItemHover(item.key, e.currentTarget)}
                    ...
                    >
                      <div className="relative pl-16">
                        <span
                          className={cn(
                            "absolute left-0 top-3 inline-block rounded-md bg-white/10 px-2 py-1 text-[11px] font-semibold tracking-[0.2em]",
            ...
            <div className="col-span-12 lg:col-span-4 flex lg:justify-end lg:relative">
              <motion.div
                className="w-full lg:max-w-[420px] rounded-2xl p-8 sm:p-9 md:p-10 lg:absolute lg:top-0"
                style={{
                  backgroundColor: blend(panelItem.color as Hex, "#1a1a1a", 0.72),
                  maxHeight: 360,
                  width: "100%",
                  transition: "background-color 220ms ease, top 300ms ease",
                  top: hoverKey ? `${panelTop}px` : 0,
                }}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {d.linkedTestCases.map((tc) => (
                          <a
                            ...
                            title="Open TestCases (demo link)"
                          >
                            {tc}
                          </a>
                        ))}
              ...
              <ul className="mt-3 space-y-2">
                {checklistLabels.map((t, idx) => (
                  ...
                  >
                    ...
                    <span
                      className={cn(checked[idx] ? "text-black/90" : "text-black/70")}
                    >
                      {t}
                    </span>
                  </li>
...
function WorkPlanerPage() {
  ...
  const pending = useMemo(
    () => todays.filter((t) => t.status === "PENDING"),
    [todays]
  );
  const done = useMemo(() => todays.filter((t) => t.status === "DONE"), [todays]);
  const failed = useMemo(
    () => todays.filter((t) => t.status === "FAILED"),
    [todays]
  );

  const greeting = useMemo(
    () => getGreeting(USER_NAME, TIME_ZONE),
    [USER_NAME, TIME_ZONE]
  );
...
function Shell() {
  ...
  // Create Test Plan has its own gradient background, so don't apply bg-white
  const isCreateTestPlan = location.pathname === "/create-test-plan";
  const isStandalonePage = isCreateTestPlan;
  ...
  return (
    <div className={!isStandalonePage ? "min-h-screen bg-white text-neutral-900" : "min-h-screen"}>
      {!isStandalonePage && <TopNav onOpen={() => setMenuOpen(true)} open={menuOpen} />}
      ...
      <main>
        <AnimatePresence mode="wait">
          <Routes>
            ...
            <Route path="/create-test-plan" element={<CreateTestPlanPage />} />
            <Route path="*" element={<Navigate to="/testcases" replace />} />
          </Routes>
        </AnimatePresence>
      </main>

      {!isStandalonePage && <MenuOverlay open={menuOpen} onClose={() => setMenuOpen(false)} />}
    </div>
  );
}
...
//   window.__RUN_QAHUB_TESTS__ = true; location.reload();
// ---------------------
function runSelfTests() {
  ...
  console.assert(
    MENU_ITEMS[1].key === "defects",
    "Second menu item should be Defect Tracker"
  );
